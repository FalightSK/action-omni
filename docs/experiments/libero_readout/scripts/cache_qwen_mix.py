...
MIX = os.environ.get("MIX_PHRASINGS", "0") == "1"   # train pool = {orig,para1,para2}
TRAIN_POOL = [0, 1, 2]                                # para3 (v=3) stays HELD OUT
TAG = "qmix" if MIX else "qtrain"
...
def run(rows, variants, out, mout):
    ...
    for vi, v in enumerate(variants):
        for n, i in enumerate(rows):
            ...
            vv = v
            if MIX and v == 0:
                vv = TRAIN_POOL[(i * 7919 + n) % len(TRAIN_POOL)]   # deterministic mix
            inp = build(instr_for(d["task_index"].iloc[i], vv), img)
            ...
            if ipos:
                start = ipos[-1] + 1
                txt_ids = tok(instr_for(d["task_index"].iloc[i], vv),
...
run(tr, [0], os.path.join(RES, f"{TAG}_feats.npy"), os.path.join(RES, f"{TAG}_mask.npy"))
...
if not MIX: np.save(os.path.join(RES, "qval_act.npy"), chunks[[cmap[i] for i in va]])
if not MIX: np.save(os.path.join(RES, "qval_state.npy"),
