...
TAG = os.environ.get("TRAIN_TAG", "qtrain")   # qtrain | qmix
H, DA, MAXI = 8, 7, 24

Xtr = np.load(os.path.join(RES, f"{TAG}_feats.npy"), mmap_mode="r")
Mtr = np.load(os.path.join(RES, f"{TAG}_mask.npy"))
Xva = np.load(os.path.join(RES, "qval_feats.npy"), mmap_mode="r")
Mva = np.load(os.path.join(RES, "qval_mask.npy"))
Atr, Str = np.load(os.path.join(RES, f"{TAG}_act.npy")), np.load(os.path.join(RES, f"{TAG}_state.npy"))
...
def run(tap_i, mode, lr=1e-3, steps=2000, bs=128, seed=0):
    ...
    torch.save(m.state_dict(), os.path.join(RES, f"qck{'mix' if TAG=='qmix' else ''}_{mode}_tap{int(TAPS[tap_i])}.pt"))
...
if __name__ == "__main__":
    ...
    for mode in ("all", "instr"):
        for i in range(len(TAPS)):
            ...
            json.dump(R, open(os.path.join(RES, f"qsweep_{TAG}.json"), "w"), indent=1)
    print("QSWEEP_DONE")
