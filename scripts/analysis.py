...
Generates 8 figures saved to <output_dir>/analysis/:
  ...
  3. attention_heatmap.png     — AttentionReadout spatial attention on 8×8 grid
...
device = torch.device("cpu")   # CPU for analysis — deterministic
OUT    = ROOT / cfg.output_dir / "analysis"
OUT.mkdir(parents=True, exist_ok=True)
print(f"Analysis: Exp{args.exp}  |  device: {device}  |  output: {OUT}")

# ── Load checkpoint ───────────────────────────────────────────────────────────
ckpt_path = ROOT / cfg.output_dir / "checkpoints" / "best.pt"
...
full_ds = PushTEmbeddingDataset(str(ROOT / cfg.embeddings_cache))
...
def collect():
    for batch in val_loader:
        ...
        # Adapted embedding (before decoder) — return context only
        if embed.ndim == 3:
            h = model.adapter(embed, img_mask)   # (B, 512)
        else:
            h = embed
        all_adapted.append(h.cpu())
        # Mean of image tokens for raw VLM rep
        if embed.ndim == 4:
            all_embeds_raw.append(embed[:, 0, :64].float().mean(dim=1).cpu())
        else:
            all_embeds_raw.append(embed[:, :64].float().mean(dim=1).cpu())
...
state_arr  = torch.cat(all_states).numpy()  # (N, state_dim)
...
# Always use position dims only (first 2) for spatial display
state_std_2d  = np.array(cfg.state_std[:2],  dtype=np.float32)
state_mean_2d = np.array(cfg.state_mean[:2], dtype=np.float32)

pred_px = pred_arr * action_std + action_mean   # (N,16,2) in pixels
gt_px   = gt_arr   * action_std + action_mean
state_px = state_arr[:, :2] * state_std_2d + state_mean_2d   # (N,2) agent pos
...
log_path = ROOT / cfg.output_dir / "train.log"
...
if epochs_tr:
    ...
    ax.text(0.5, 0.5, "train.log not found", ha="center", va="center",
            transform=ax.transAxes)
ax.set_xlabel("Epoch"); ax.set_ylabel("CFM Loss")
ax.set_title(f"Exp{args.exp}: Training & Validation Loss"); ax.legend(); ax.grid(alpha=0.3)
...
plt.suptitle(f"Exp{args.exp}: Prediction Quality Analysis", fontsize=13, y=1.01)
...
class AttentionReadoutWithWeights(nn.Module):
    ...
    def forward(self, tokens, key_padding_mask=None):
        ...
        return self.norm(out.squeeze(1)), weights.squeeze(1)   # (B, 82)
...
with torch.no_grad():
    for batch in val_loader:
        ...
        # Fuse multi-scale if needed, then LoRA + SpatialAwareMLP
        if embed.ndim == 4 and model.adapter.fusion is not None:
            embed = model.adapter.fusion(embed)
        elif embed.ndim == 4:
            embed = embed[:, 0]   # fallback: take first layer
        h = model.adapter.lora(embed)
        h = model.adapter.spatial(h, img_mask, cfg.img_grid_h, cfg.img_grid_w)
        _, weights = readout_patched(h)

        all_attn_weights.append(weights.cpu())
        all_img_masks_val.append(img_mask.cpu())
        if len(all_attn_weights) >= 5:
            break

attn_w        = torch.cat(all_attn_weights).numpy()
img_masks_val = torch.cat(all_img_masks_val).numpy()
...
mean_attn_map = img_attn.mean(0).reshape(8, 8)
...
axes[0].set_title(f"Mean Attention — Image Tokens (8×8)\n"
...
plt.suptitle(f"Exp{args.exp}: AttentionReadout — Where Does the Model Look?", fontsize=13)
plt.tight_layout()
...
ax = fig.add_subplot(gs[0, 0])
ax.hist2d(pred_px[:, 0, 0], pred_px[:, 0, 1],
...
plt.suptitle(f"Exp{args.exp}: Action Distribution & Decision Boundary", fontsize=13)
...
plt.suptitle(f"Exp{args.exp}: Embedding Space — Raw VLM vs Adapted", fontsize=13)
plt.tight_layout()
...
@torch.no_grad()
def get_flow_trajectory(model, embed, state, img_mask, n_steps=20):
    """
    Sample with many steps and record the denoising path.
    Works for both MLP (Exp1) and DiT (Exp2) decoders.
    Returns (n_steps+1, B, H, 2) — full action chunk at each step.
    """
    B = embed.shape[0]
    cond_vec, vlm_tokens = model._build_cond(embed, state, img_mask)

    if model.use_dit:
        # DiT: (B, H, action_dim)
        x = torch.randn(B, cfg.action_horizon, cfg.action_dim)
        traj = [x[:, 0].clone()]   # track first action step only
        dt = 1.0 / n_steps
        for i in range(n_steps):
            t = torch.full((B,), i / n_steps)
            v = model.decoder.forward(x, t, cond_vec, vlm_tokens)
            x = x + v * dt
            traj.append(x[:, 0].clone())   # (B, 2)
    else:
        # MLP: (B, H*D) flat
        flat_dim = cfg.action_dim * cfg.action_horizon
        x = torch.randn(B, flat_dim)
        traj = [x[:, :2].clone()]
        dt = 1.0 / n_steps
        for i in range(n_steps):
            t = torch.full((B,), i / n_steps)
            v = model.decoder.forward(x, t, cond_vec)
            x = x + v * dt
            traj.append(x[:, :2].clone())   # (B, 2)

    return torch.stack(traj, dim=0)  # (n_steps+1, B, 2) — first action step

# Pick 8 representative val samples
batch    = next(iter(DataLoader(val_ds, batch_size=8, shuffle=True)))
embed_b  = batch["embedding"].float()
state_b  = batch["state"]
img_mask_b = batch.get("img_mask")
gt_np    = batch["actions"].float().numpy()   # (8, H, 2)

traj = get_flow_trajectory(model, embed_b, state_b, img_mask_b, n_steps=20)
traj = traj.numpy()   # (21, 8, 2) — first action step only
...
for s_idx, ax in enumerate(axes.flat):
    ...
    gt_x   = gt_np[s_idx, 0, 0] * action_std[0] + action_mean[0]
    gt_y   = gt_np[s_idx, 0, 1] * action_std[1] + action_mean[1]
...
decoder_type = "DiT" if model.use_dit else "MLP"
plt.suptitle(f"Exp{args.exp} ({decoder_type}): Flow Matching Denoising Trajectory (t+1 action, 20 steps)\n"
...
# Group by component — handles both MLP and DiT decoder naming
group_vals = {
    "LoRA A":             [],
    "LoRA B":             [],
    "SpatialMLP":         [],
...
n_blocks = cfg.dit_num_layers if model.use_dit else cfg.decoder_num_layers
...
for name, val in grad_data.items():
    if "lora.A" in name:
        ...
        if m:
            bi = int(m.group(1))
            if bi < third:
                group_vals["Decoder (early)"].append(val)
            elif bi < 2 * third:
                group_vals["Decoder (mid)"].append(val)
            else:
                group_vals["Decoder (late)"].append(val)
        else:
            group_vals["Decoder (early)"].append(val)

labels = [k for k in group_vals if group_vals[k]]
means  = [np.mean(group_vals[k]) for k in labels]
...
axes[0].set_title(f"Exp{args.exp}: Gradient Flow — Mean |grad| per Layer Group")
...
names_top = [n.replace("decoder.blocks.", "dec.b").replace("adapter.", "adp.") for n, _ in all_items]
...
sim_path = ROOT / cfg.output_dir / "sim_results.json"
...
if sim_path.exists():
    ...
    replan_list = [e.get("replan_count", 0) for e in eps]
    ...
    sr = np.mean(succ_list) * 100
    ax.set_title(f"Coverage per Episode\n(green=success, red=fail)  SR={sr:.0f}%")
    ...
    ax = axes[2]
    failed_gaps = [max(0, (0.95 - c) * 100) for c, s in zip(cov_list, succ_list) if not s]
    if failed_gaps:
        ax.hist(failed_gaps, bins=15, color="#DD8452", edgecolor="white",
                label=f"Failed eps (n={len(failed_gaps)})")
    ...
    for ax in axes:
        ax.text(0.5, 0.5, f"sim_results.json not found\n({sim_path})",
                ha="center", va="center", transform=ax.transAxes, fontsize=9)

plt.suptitle(f"Exp{args.exp}: Simulation Coverage Analysis", fontsize=13)
plt.tight_layout()
